/// Taken from rust analyzer at https://github.com/rust-lang/rust-analyzer
use std::{
    cell::{Cell, RefCell},
    path::{Path, PathBuf},
    time::Duration,
...
impl Project<PathBuf> {
    ...
    pub(crate) fn server(self) -> Server {
        Server::new(self.kinetic_model_path)
    }
}
...
impl Server {
    fn new(kinetic_model_path: PathBuf) -> Server {
        ...
        let config_to_main = config.clone();
        let _thread = jod_thread::Builder::new()
            .name("test server".to_string())
            .spawn(move || main_loop(connection, config_to_main).unwrap())
    ...
    // pub(crate) fn wait_until_workspace_is_loaded(self) -> Server<P> {
